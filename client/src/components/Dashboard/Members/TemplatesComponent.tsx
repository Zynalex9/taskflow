...
export const TemplatesComponent = () => {
  ...
  const handleCreateBoard = async () => {
    if (!selectedWorkspace?._id || !selectedTemplate?._id || !boardName) return;
    ...
    <div className="flex items-center gap-2 flex-wrap">
      {isLoading &&
        Array.from({ length: 4 }).map((_, idx) => (
          <Skeleton
            key={idx}
            className="w-[200px] h-[120px] rounded-lg bg-bgS/50"
          />
        ))}
      ...
      <AlertDialog open={openDialog} onOpenChange={setOpenDialog}>
        <AlertDialogContent className="bg-bgS border-none text-textP">
          <AlertDialogHeader>
            <AlertDialogTitle>
              Create a new board from '{selectedTemplate?.title}' template
            </AlertDialogTitle>
            ...
            <AlertDialogDescription className="space-y-4">
              ...
              <AlertDialogFooter>
                ...
                <button
                  type="button"
                  disabled={!selectedWorkspace || !boardName.trim() || loading}
